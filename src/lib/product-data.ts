...
export const sampleProducts: Product[] = [
  {
    ...
    seo: {
      metaTitle: "Michelin Pilot Sport 4S 225/45R17 - Premium Summer Tire",
      metaDescription: "Buy Michelin Pilot Sport 4S 225/45R17 summer tire. Exceptional grip, handling and performance for sports cars. Free shipping available.",
      keywords: ["michelin", "pilot sport", "summer tire", "225/45r17", "sports car"],
      slug: "michelin-pilot-sport-4s-225-45r17"
    },
...
export const productAPI = {
  async getProducts(filter: ProductFilter = {}): Promise<{ products: Product[]; total: number; stats: ProductStats }> {
    ...
    const stats: ProductStats = {
      ...
      lowStockProducts: sampleProducts.filter(p => p.inventory.availableQuantity <= p.inventory.reorderLevel).length,
      outOfStockProducts: sampleProducts.filter(p => p.inventory.availableQuantity === 0).length,
      ...
      totalValue: sampleProducts.reduce((sum, p) => sum + (p.pricing.basePrice * p.inventory.stockQuantity), 0),
      avgPrice: sampleProducts.reduce((sum, p) => sum + p.pricing.basePrice, 0) / sampleProducts.length
    };
